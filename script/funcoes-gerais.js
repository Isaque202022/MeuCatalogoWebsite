...
function filtrarProdutos(naoRenderizar){
  ...
  for (let p of produtos) {
    ...
    p.referenciaCruzada.forEach(ref => {
      ...
      ref.codigos.forEach(cod => {
        if (cod.valor.toLowerCase().replace(/[^a-zA-Z0-9]/g,'').includes(filtros.codCruzado.toLowerCase().replace(/[^a-zA-Z0-9]/g,''))) {
    ...
    if (!encontradoPesqCod && filtros.codCruzado != '' && !p.referencia.toLowerCase().trim().replace(/[^a-zA-Z0-9]/g,'').includes(filtros.codCruzado.toLowerCase().trim().replace(/[^a-zA-Z0-9]/g,''))) {
      p.visivel = false
      continue
    }
    ...
    // ENCAIXE
    encontrado = false
    if (filtros.propriedades.quantTerminais == p.propriedades.quantTerminais){
      encontrado = true
    }
    if (filtros.propriedades.quantTerminais ==0) encontrado = true
    if (!encontrado){
      p.visivel = false
      continue
    }
  ...
  // mostrar quantos filtros foram aplicados
  quantFiltros = 0
  if (filtros.marca.length > 0) quantFiltros +=1
  if (filtros.referencia.length > 0) quantFiltros +=1
  if (filtros.codCruzado.length > 0) quantFiltros +=1
  if (filtros.descricao[0].length > 0) quantFiltros +=1
  if (filtros.descricao[1].length > 0) quantFiltros +=1
  if (filtros.aplicacao.montadoras.length > 0) quantFiltros +=1
  if (filtros.aplicacao.veiculos.length > 0) quantFiltros +=1
  if (filtros.aplicacao.ano.length > 0) quantFiltros +=1
  if (filtros.propriedades.quantTerminais > 0) quantFiltros +=1
  document.getElementById('contagem-filtros').textContent = quantFiltros

  contarOcorrenciaFiltros()
}

/*
function atualizarOptionsDosFiltros() {
	// encaixe
	let encaixes = {}
	encaixes.options = Array.from(f_select_encaixe.children)
	encaixes.lista = []
	
	for (let o of encaixes.options) encaixes.lista.push(Number(o.value))
	for (let e of encaixes.options) {
		if (e.value) e.disabled = true
	}
	
	for (let p of produtos){
		let index = encaixes.lista.indexOf(p.propriedades.quantTerminais)
		if (index!= -1 && p.propriedades.quantTerminais != 0 && p.visivel){
			encaixes.options[index].disabled = false
		}
	}
}*/
...
function ordenarListaDeAplicacoes(aplicacao){
  ...
  for (let app of aplicacao) {
    app.carros.sort((a, b) => {
      ...
      let comp = carroA.localeCompare(carroB)
      if (comp !== 0) return comp
      // ano inicio (mais antigo primeiro)
      let anoA = Number(a.anoInicio) || 0
      let anoB = Number(b.anoInicio) || 0
      if (anoA !== anoB) {
        return anoA - anoB
      }
      // motor
      let motorComp = (a.motor || '').localeCompare(b.motor || '')
      if (motorComp !== 0) return motorComp
      // complemento
...
function renderizarTabelaProd() {
  ...
  labelQuantItens.innerText = contarVisiveis
  ...
  if (bodyTableProd.innerHTML == ''){
    ...
    //aplic_carros_tbody.innerHTML = gerarLinhasAplicacao([])
...
function gerarLinhasAplicacao(aplicacoes) {
  ...
  aplicacoes.forEach(aplic => {
    ...
    aplic.carros.forEach(carro => {
      ...
      let negrito = veiculosFiltro.some(v => v.includes(nomeCarro))

      const anoFiltro = filtros.aplicacao.ano

      if (anoFiltro !== '') {

        const ano = Number(anoFiltro)
        const inicio = Number(carro.anoInicio)
        const fim = Number(carro.anoFim)

        if (inicio && fim) {
          negrito = negrito && (ano >= inicio && ano <= fim)
        }
        else if (inicio) {
          negrito = negrito && (ano >= inicio)
        }
        else if (fim) {
          negrito = negrito && (ano <= fim)
        }
...
function toggleModalFiltrar() {
  ...
  for (let l of Array.from(bodyTableProd.children)){
    if (l.id == prodSelecionado.elemento.id) {
      l.scrollIntoView({
        block: "center",
...
function renderizarMontadorasSelecionadas() {
  ...
  if (listaMontadorasSelecionadas.length){
    inputMontadoraHandcraft.closest('tr').querySelector('.resetar-campo').classList.remove('invisivel')
  } else {
    inputMontadoraHandcraft.closest('tr').querySelector('.resetar-campo').classList.add('invisivel')
  }
}
...
function carregarListaVeiculos(){
  ...
  for (let p of produtos) {
    for (let ap of p.aplicacao){
      ...
      ap.carros.forEach(carro => {
        let nome = carro.nome.toUpperCase().trim()
...
function renderizarVeiculosSelecionadas() {
  ...
  listaVeiculosSelecionadas.forEach(v =>{
    ...
    item.addEventListener('click', ()=>{
      excluirVeiculo(v)
      filtrarProdutos()
      inputVeiculoHandcraft.focus()
      f_select_ano.selectedIndex = 0
      var event = new Event('change', { bubbles: true });
      f_select_ano.dispatchEvent(event);
    })
  })
  if (listaVeiculosSelecionadas.length){
    inputVeiculoHandcraft.closest('tr').querySelector('.resetar-campo').classList.remove('invisivel')
  } else {
    inputVeiculoHandcraft.closest('tr').querySelector('.resetar-campo').classList.add('invisivel')
  }
}
    ...
    for (let aplic of p.aplicacao){
      for (let carro of aplic.carros){
        ...
        if (carroLoop == carroPesquisa){
          // ANO INICIO
          let anoinicio = carro.anoInicio.replaceAll(' ','')
          anoinicio = anoinicio.match(/(\d{4})$/)
          if (anoinicio){
          ...
          if (menorAno == 0) menorAno = anoinicio
          else if (anoinicio < menorAno) menorAno = anoinicio
          // ANO FIM
          let anofim = carro.anoFim.replaceAll(' ','')
          anofim = anofim.match(/(\d{4})$/)
          if (anofim){
...
function renderizarMarcasSelecionadas() {
  ...
  listaMarcasProdSelected.forEach(m =>{
    ...
    <p style='cursor: pointer;'>
      <span class="quant" style='cursor: pointer;'></span>
      <span class="nome-marca" style='cursor: pointer;'>${m.toUpperCase()}</span>
    </p>
    <p>
      <img style='width: 18px; cursor: pointer;' src='imagens/pagina/icone-lixeira.png'>
    </p>`
    item.title = 'Excluir'
    ...
    item.addEventListener('click', ()=>{
      excluirMarca(m)
      filtrarProdutos()
      if (listaMarcasProdSelected.length==0){
        f_select_marca.closest('tr').querySelector('.resetar-campo').classList.add('invisivel')
      }
    })
...
function limparTodosOsFiltros(){
  document.querySelectorAll('.resetar-campo').forEach(e =>{
  ...
  // encaixe
  document.getElementById('select-encaixe').selectedIndex = 0;
  filtros.propriedades.quantTerminais = 0;
  // marcas
...
let encaixes = []
for (let p of produtos){
  let n = p.propriedades.quantTerminais
  if (n==0) continue
  if (!encaixes.includes(n)) {
    encaixes.push(p.propriedades.quantTerminais)
  }
}
encaixes.sort()
f_select_encaixe.innerHTML = '<option value="">Todos</option>'
encaixes.forEach(e =>{
  const option = document.createElement('option')
  option.value = e
  option.textContent = e 
  if (e==1)  option.textContent += ' pino'
  if (e==1)  option.innerHTML += ' <span></span>'
  
  else {
    option.textContent += ' pinos'
    option.innerHTML += ' <span></span>'
  }
  f_select_encaixe.appendChild(option)
})


function contarOcorrenciaFiltros(){
  for (let e of Array.from(f_select_encaixe.children)){
    if (!e.value) continue 
    quantOcorrenciaFiltros = 0
    let antes = filtros.propriedades.quantTerminais
    filtros.propriedades.quantTerminais = Number(e.value)
    filtrarProdutos(true)
    for (let p of produtos){
      if (p.visivel) quantOcorrenciaFiltros += 1
    }
    filtros.propriedades.quantTerminais = antes
    filtrarProdutos(true)

    e.querySelector('span').innerText = `(${quantOcorrenciaFiltros})`
  }
  for (let e of Array.from(f_select_marca.children)){
    if (!e.value) continue 
    quantOcorrenciaFiltros = 0
    let antes = filtros.marca
    filtros.marca = [e.value]
    filtrarProdutos(true)
    for (let p of produtos){
      if (p.visivel) quantOcorrenciaFiltros += 1
    }
  ...
  for (let e of Array.from(containerMarcasSelecionadas.children)){
    quantOcorrenciaFiltros = 0
    let antes = filtros.marca
    let valor = e.querySelector('.nome-marca').innerText
    filtros.marca = [valor]
    filtrarProdutos(true)
    for (let p of produtos){
      if (p.visivel) quantOcorrenciaFiltros += 1
    ...
    e.querySelector('.quant').innerText = `(${quantOcorrenciaFiltros})`
  }
  for (let e of Array.from(dropMontadoraHandcraft.children)){
